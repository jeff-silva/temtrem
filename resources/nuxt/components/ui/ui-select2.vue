...
<template>
    <div class="ui-select2" style="position:relative;">
...
<style>
.ui-select2 * {transition: all 300ms ease;}
.ui-select2 > .form-control {cursor:pointer;}
</style>
...
export default {
  ...
  methods: {
      ...
      getOptions() {
        ...
        this.$el.querySelectorAll('.ui-select2-options>*').forEach(item => {
          item.classList.add('ui-select2-option');
          items.push({
            html: item.innerHTML,
            text: item.innerText,
          });
