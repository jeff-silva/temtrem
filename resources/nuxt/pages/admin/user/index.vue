<template><div>
	<form class="input-group mb-3" @submit.prevent="getResults()" style="max-width:300px;">
		<input type="text" class="form-control border-0" v-model="params.search" placeholder="Buscar Usuários" />
	...
	<ui-actions>
		<nuxt-link to="/admin/user/0" class="btn btn-primary">Novo</nuxt-link>
	</ui-actions>
</div></template>
...
export default {
	...
	methods: {
		getResults() {
			...
			this.$axios('/api/user/search', {params:this.params}).then(resp => {
		...
		deleteItem(item) {
			this.$confirm({title:'Deseja deletar user?'}).then(resp => {
				this.$axios.post(`/user/delete/${item.id}`).then(resp => {
					this.$swal('user deletado', '', 'success');
				});
			});
		},
	},
